		...
		try {
			...
			if (progressDialog != null) {
				currentActivity.runOnUiThread(new ChangeMessage(progressDialog,
						"Découpage du fichier stopTimes.txt pour chargement opportuniste"));
			}
			...
			LOG_YBO.debug("Lecture du fichier et construction des relations Arret - Route.");
			...
			while ((ligne = bufReader.readLine()) != null) {
				...
				if (mapTripIdFichier.containsKey(tripId)) {
					...
					if (!stops.contains(stopId)) {
						...
						dataBaseHelper.insert(arretRoute);
					}
