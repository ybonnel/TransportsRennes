...
@SuppressWarnings({"ConstantNamingConvention", "LocalVariableNamingConvention"})
public class CalculDistance {
	...
	public CalculDistance(final double lat1, final double lon1, final double lat2, final double lon2) {
		super();
		this.lat1 = lat1;
		this.lon1 = lon1;
		this.lat2 = lat2;
	...
	private double iterationCalcul(final double lambda) {
		...
		final double sinSigma = Math.sqrt(sinSqSigma);
		final double cosSigma = sinU1sinU2 + cosU1cosU2 * cosLambda; // (15)
		sigma = Math.atan2(sinSigma, cosSigma); // (16)
		final double sinAlpha = sinSigma == 0 ? 0.0 : cosU1cosU2 * sinLambda / sinSigma; // (17)
		final double cosSqAlpha = 1.0 - sinAlpha * sinAlpha;
		final double cos2SM = cosSqAlpha == 0 ? 0.0 : cosSigma - 2.0 * sinU1sinU2 / cosSqAlpha; // (18)
