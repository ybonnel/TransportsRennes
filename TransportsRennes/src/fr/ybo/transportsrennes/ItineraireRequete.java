...
public class ItineraireRequete extends MenuAccueil.Activity implements LocationListener {
	...
	private void traiterAdresseMultiple(GeocodeResponse reponseDepart, GeocodeResponse reponseArrivee) {
		Toast.makeText(this, "Plusieurs adresses trouvées, veuillez préciser l'adresse", Toast.LENGTH_LONG);
	}

	private void calculItineraire(GeocodeResponse reponseDepart, GeocodeResponse reponseArrivee) {
		...
		ItineraireReponse reponse = CalculItineraires.getInstance().calculItineraires(adresseDepart, adresseArrivee, calendar);
		if (reponse.erreur != null) {
			Toast.makeText(this, R.string.erreur_calculItineraires, Toast.LENGTH_LONG).show();
		} else {
